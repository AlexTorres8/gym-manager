...
function App() {
  ...
  const handleDeleteClient = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/clients/${editingClient.id}`, { method: 'DELETE' })
      if (response.ok) {
  ...
  if (!isAuthenticated) {
    return (
        <div style={{ 
            height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f3f4f6' 
        }}>
            <form onSubmit={handleLogin} style={{ 
                background: 'white', padding: '40px', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', textAlign: 'center', width: '300px'
            }}>
                <h2 style={{ marginBottom: '20px', color: '#333' }}>🔐 Acceso Gimnasio</h2>
                <input type="password" placeholder="Contraseña..." value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} style={{ width: '100%', padding: '12px', marginBottom: '20px', border: '1px solid #ddd', borderRadius: '6px', fontSize: '16px' }} />
                <button type="submit" style={{ width: '100%', padding: '12px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer' }}>Entrar</button>
            </form>
        </div>
    )
  }
    ...
    <div className="main-container">
      ...
      <div className="nav-header">
          ...
          <div className="nav-buttons-group">
            <button onClick={() => setCurrentView('reception')} className="nav-btn" style={{ background: currentView === 'reception' ? '#111827' : '#e5e7eb', color: currentView === 'reception' ? 'white' : '#374151' }}>Recepción</button>
            <button onClick={() => setCurrentView('list')} className="nav-btn" style={{ background: currentView === 'list' ? '#111827' : '#e5e7eb', color: currentView === 'list' ? 'white' : '#374151' }}>Socios 📋</button>
            <button onClick={() => setCurrentView('stats')} className="nav-btn" style={{ background: currentView === 'stats' ? '#111827' : '#e5e7eb', color: currentView === 'stats' ? 'white' : '#374151' }}>Estadísticas 📊</button>
          </div>
          <button onClick={handleLogout} style={{ marginLeft: '10px', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '1.2rem' }} title="Cerrar Sesión">🔓</button>
      </div>
      ...
      {currentView === 'reception' && (
        <div style={{ animation: 'fadeIn 0.3s' }}>
           ...
           <input type="text" placeholder="Buscar socio (Nombre o DNI)..." value={query} onChange={(e) => setQuery(e.target.value)} className="search-input" />
                  ...
                  <div key={user.id} className="user-card" style={{ borderLeft: `5px solid ${getStatusTextColor(user.status)}` }}>
                      <div className="card-info">
                          <h3 style={{ margin: '0 0 5px 0', color: '#333' }}>
                              ...
                              <span style={{fontSize:'0.8em', color:'#666', marginLeft:'10px'}}>({user.dni || 'Sin DNI'})</span>
                          </h3>
                      ...
                      <div className="card-actions">
                         ...
                         <div style={{ padding: '5px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold', backgroundColor: getStatusColor(user.status), color: getStatusTextColor(user.status) }}>{user.status}</div>
                         <button onClick={() => { const dateStr = user.last_expiration_date ? new Date(user.last_expiration_date).toISOString().split('T')[0] : ''; setEditingClient({ ...user, expiration_date: dateStr }); }} className="nav-btn" style={{ background: '#4b5563', color: 'white' }}>Datos ✏️</button>
           ...
           <div style={{ marginTop: '40px', borderTop: '2px solid #eee', paddingTop: '20px' }}>
                ...
                {visits.length === 0 ? <p style={{ color: '#888' }}>Nadie ha entrado recientemente.</p> : (
                    <table className="visits-table">
                        <thead><tr style={{ background: '#f8f9fa' }}><th>Hora</th><th>Socio</th></tr></thead>
                        <tbody>
                            {visits.map(v => (
                                <tr key={v.id}>
      ...
      {currentView === 'stats' && stats && (
        <div style={{ animation: 'fadeIn 0.5s' }}>
            <div className="stats-kpi-container">
                <div className="kpi-card"><h3 style={{ margin: 0, color: '#666' }}>Socios Totales</h3><p style={{ fontSize: '40px', fontWeight: 'bold', margin: '10px 0', color: '#2563eb' }}>{stats.totals.total_clients}</p></div>
                <div className="kpi-card"><h3 style={{ margin: 0, color: '#666' }}>Activos Ahora</h3><p style={{ fontSize: '40px', fontWeight: 'bold', margin: '10px 0', color: '#16a34a' }}>{stats.totals.active_clients}</p></div>
            </div>
                ...
                <div style={{ height: '300px' }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={stats.daily}>
                            <CartesianGrid strokeDasharray="3 3" /> <XAxis dataKey="name" /> <YAxis allowDecimals={false} /> <Tooltip contentStyle={{ color: '#333' }} />
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleCreateSubscription} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <select className="form-input" required value={selectedPlanId} onChange={(e) => setSelectedPlanId(e.target.value)} style={{ backgroundColor: 'white' }}>
                ...
                {plans.map(plan => (<option key={plan.id} value={plan.id}>{plan.name} - {plan.price}€ ({plan.duration_days} días)</option>))}
              </select>
              <div style={{ display: 'flex', gap: '10px' }}>
